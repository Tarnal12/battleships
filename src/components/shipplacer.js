...
export function placeShipAtRandom(existingShips, gridSize, shipLength, shipId) {
    ...
    while (!placed) {
        ...
        const startX = isVertical ? utils.RandNum(gridSize) : utils.RandNum(gridSize - shipLength + 1);
        const startY = isVertical ? utils.RandNum(gridSize - shipLength + 1) : utils.RandNum(gridSize);

        const targetIndices = getTargettedIndices(startX, startY, isVertical, shipLength, gridSize);
